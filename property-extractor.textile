...
pre cpan -i CSS
pre cpan -i Getopt::Long


Under Linux Debian, you can also use the native installer:
pre apt-get install libcss-perl
pre apt-get install libgetopt-long-descriptive-perl
...
h3. To remove one property from an element ([http://api.jquery.com/css/]):
